/* PrismJS 1.30.0
https://prismjs.com/download#themes=prism&languages=markup+css+clike+javascript+antlr4+awk+bash+bnf+c+csharp+cpp+cmake+css-extras+csv+dart+diff+docker+erlang+git+linker-script+go+go-module+graphql+groovy+haskell+http+hpkp+hsts+icon+ignore+ini+java+javadoc+javadoclike+js-extras+json+json5+jsstacktrace+js-templates+kotlin+latex+less+lisp+llvm+log+lua+makefile+markdown+markup-templating+matlab+mermaid+nginx+opencl+perl+plant-uml+plsql+powershell+protobuf+python+qml+qore+ruby+rust+sql+swift+tt2+toml+typescript+typoscript+uri+verilog+vhdl+vim+wasm+wiki+xml-doc+yaml+zig&plugins=line-highlight+line-numbers+autolinker+wpd+file-highlight+jsonp-highlight+highlight-keywords+inline-color+previewers+autoloader+keep-markup+command-line+normalize-whitespace+data-uri-highlight+match-braces+diff-highlight+filter-highlight-all+treeview */
